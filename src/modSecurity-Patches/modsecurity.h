...
typedef struct {
  ...
  apr_thread_mutex_t *redis_local_lock;
  apr_thread_mutex_t *redis_master_lock;
} msc_server_config;
...
redisContext* msc_connect_and_authenticate_to_redis(char* redis_ip,
						int redis_port,
						char* redis_password);

redisContext* msc_connect_to_redis_unix_socket(void);

redisContext* msc_connect_to_redis_master(request_rec* r,
				      redisContext* redis_local_connection,
				      char* redis_password);
