...
/**
 * Cleanup for configuration
 */
static apr_status_t config_pool_cleanup(void *param) {
    msc_server_config *conf = (msc_server_config*) param;

    ap_log_error_(APLOG_MARK, APLOG_DEBUG, 0, NULL, "Mod_security::config_pool_cleanup: Cleaning pool config");
    if( !conf )
        return APR_SUCCESS;

    /* Lock the mutex */
    if( conf->redis_local_lock )
        apr_thread_mutex_lock(conf->redis_local_lock);
    if (conf->redis_master_lock && conf->redis_master_lock != conf->redis_local_lock)
	apr_thread_mutex_lock(conf->redis_master_lock);
    /* Free redis master connection */
    if( conf->redis_master_conn ) {
        ap_log_error(APLOG_MARK, APLOG_NOTICE, 0, NULL, "Mod_security:Closing master REDIS connection");
    ...
    /* Unlock mutex */
    if( conf->redis_local_lock )
        apr_thread_mutex_unlock(conf->redis_local_lock);
    if (conf->redis_master_lock && conf->redis_master_lock != conf->redis_local_lock)
	apr_thread_mutex_unlock(conf->redis_master_lock);
  return APR_SUCCESS;
}
 ...
 */
static void hook_child_init(apr_pool_t *mp, server_rec *server_base) {
    modsecurity_child_init(modsecurity);


    int threaded_mpm = 0;

    ap_log_error_(APLOG_MARK, APLOG_DEBUG, 0, NULL, "Mod_security::hook_child_init: Creating connection to local REDIS");
    /* Connect to redis by unix socket (REDIS_SOCKET) */
    redisContext *redis_slave_conn = msc_connect_to_redis_unix_socket();
    if( redis_slave_conn->err != REDIS_OK ) {
	ap_log_error(APLOG_MARK, APLOG_ALERT, 0, NULL, "ModSecurity::hook_child_init: Cannot connect to slave REDIS, "
	    "verify if it is correctly up");
    }

    ap_log_error_(APLOG_MARK, APLOG_DEBUG, 0, NULL, "Mod_security::create_srv_conf: Creating connection to master REDIS");
    /* Connect to redis master : slave_conn is ROLE=MASTER, else connect by TCP */
    redisContext *redis_master_conn = msc_connect_to_redis_master(NULL, redis_slave_conn, NULL);
    if( redis_master_conn->err != REDIS_OK ) {
	ap_log_error(APLOG_MARK, APLOG_ALERT, 0, NULL, "ModSecurity::hook_child_init: Cannot connect to master REDIS, "
	    "verify if it is correctly up");
    }
    /* Copy the initiate connection for each server_rec configuration */
    server_rec *s = NULL;
    msc_server_config *conf = NULL;
    apr_thread_mutex_t *local_lock = NULL;
    apr_thread_mutex_t *master_lock = NULL;
    ap_mpm_query(AP_MPMQ_IS_THREADED, &threaded_mpm);
    if( threaded_mpm ) {
	apr_thread_mutex_create(&local_lock,
				APR_THREAD_MUTEX_DEFAULT, mp);
	if (redis_slave_conn == redis_master_conn)
	    master_lock = local_lock;
	else
	    apr_thread_mutex_create(&master_lock,
				    APR_THREAD_MUTEX_DEFAULT, mp);
    }
    for( s=server_base ; s ; s=s->next ) {
	conf = ap_get_module_config(s->module_config, &security2_module);
	conf->redis_slave_conn = redis_slave_conn;
	conf->redis_master_conn = redis_master_conn;
	conf->redis_local_lock = local_lock;
	conf->redis_master_lock = master_lock;
    }
    /* Register cleanup that will be performed when pool childs will be destroyed */
    apr_pool_cleanup_register(mp, conf, config_pool_cleanup, apr_pool_cleanup_null);
}
 ...
 */
void* create_srv_conf(apr_pool_t* pool, server_rec* srec) {

    msc_server_config* srv_cfg = apr_palloc(pool, sizeof(msc_server_config));

    ap_log_error_(APLOG_MARK, APLOG_DEBUG, 0, NULL, "Mod_security::create_srv_conf: Creating server config...");
    if (srv_cfg) {
	...
	srv_cfg->redis_master_conn = NULL;
	srv_cfg->redis_slave_conn = NULL;
    }
    else
	ap_log_error_(APLOG_MARK, APLOG_ERR, 0, NULL, "Mod_security::create_srv_conf: Cannot create server config: Allocation failed");
...
void *merge_srv_conf(apr_pool_t *pool, void *BASE, void *ADD) {
    msc_server_config *base = (msc_server_config *) BASE ;
    msc_server_config *conf = (msc_server_config *) apr_pcalloc(pool, sizeof(msc_server_config));
    (void)ADD;

    ap_log_error_(APLOG_MARK, APLOG_DEBUG, 0, NULL, "Mod_security::merge_srv_conf: Merging server config...");
	...
	if( !conf->redis_local_lock )
	    conf->redis_local_lock = base->redis_local_lock;
	if( !conf->redis_master_lock )
	    conf->redis_master_lock = base->redis_master_lock;
    }
    else
	ap_log_error_(APLOG_MARK, APLOG_ERR, 0, NULL, "Mod_security::merge_srv_conf: Cannot merge server config: Allocation failed");
