    ...
    msre_rule *rule, msre_action *action)
{
    ...
    var->name_len = (unsigned int)strlen(var->name);

    /* Expand macros in value */
    var->value = strndup(var_value, MAX_STRING_LEN);
    var->value_len = (unsigned int)strlen(var->value);
    ...
    const char *col_name, const char *col_key, unsigned int col_key_len)
{
    ...
    ap_log_error_(APLOG_MARK, APLOG_DEBUG, 0, NULL, "Mod_security::init_collection: Try to retrieve collection from REDIS");
    /* Check if the table already exists collection from storage. */
    table = collection_retrieve(msr, real_col_name, col_key);
    ...
    if (table == NULL) {
        ...
        table = apr_table_make(msr->mp, 24);
        if (table == NULL) {
            msr_log(msr, 1, "Error when create table for collection (name "
		"\"%s\", key \"%s\").", real_col_name, col_key);
		    return -1;
	    }
