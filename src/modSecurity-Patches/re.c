...
char *msre_format_metadata(modsec_rec *msr, msre_actionset *actionset) {
    ...
    if (actionset->msg != NULL) {
        ...
        char* escaped_msg = log_escape_ex(msr->mp, var->value, var->value_len);
        msg = apr_psprintf(msr->mp, " [msg \"%s\"]", escaped_msg);

        char *reasons = (char*) apr_table_get(msr->r->subprocess_env, "reasons");
        if (!reasons) {
            reasons = apr_psprintf(msr->mp, "[\\\"%s\\\"]", escaped_msg);
        }
        else if (!strstr(reasons, escaped_msg)) {
            reasons = apr_psprintf(msr->mp, "%.*s,\\\"%s\\\"]", strlen(reasons) - 1, reasons, escaped_msg);
        }
        apr_table_set(msr->r->subprocess_env, "reasons", reasons);
    }
    ...
    char *owasptop10 = (char*) apr_table_get(msr->r->subprocess_env, "owasp_top10");
    for (k = 0; k < tarr->nelts; k++) {
        ...
        if (strcmp(telts[k].key, "tag") == 0) {
            ...
            if (var->value && strncmp(var->value, "OWASP_TOP_10/", 13) == 0) {
                char* owasptag = var->value + 13;

                if (!owasptop10) {
                    owasptop10 = apr_psprintf(msr->mp, "[\\\"%s\\\"]", owasptag);
                }
                else if (!strstr(owasptop10, owasptag)) {
                    owasptop10 = apr_psprintf(msr->mp, "%.*s,\\\"%s\\\"]", strlen(owasptop10) - 1, owasptop10, owasptag);
                }
            }
        }
    }
    apr_table_set(msr->r->subprocess_env, "owasp_top10", owasptop10);
